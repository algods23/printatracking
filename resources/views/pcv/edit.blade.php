...
@section('title', 'Edit PCV')
@section('page-title', 'Edit PCV')
@section('page-subtitle', 'Update a petty cash voucher')
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <form action="{{ route('pcv.update', $pcv) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="pcv_name" name="pcv_name" value="{{ old('pcv_name', $pcv->pcv_name) }}" required class="w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('pcv_name') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <input type="number" id="amount" name="amount" value="{{ old('amount', $pcv->amount) }}" required step="0.01" min="0" class="w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('amount') border-red-500 @enderror">
                    @error('amount')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="date" class="block text-sm font-medium text-gray-900 dark:text-white mb-2">Date *</label>
                    <input type="date" id="date" name="date" value="{{ old('date', $pcv->date->toDateString()) }}" required class="w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('date') border-red-500 @enderror">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" placeholder="Enter PCV details..." class="w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 @error('description') border-red-500 @enderror">{{ old('description', $pcv->description) }}</textarea>
            ...
            <div class="flex gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <a href="{{ route('pcv.show', $pcv) }}" class="flex-1 px-6 py-2 bg-gray-300 dark:bg-gray-700 text-gray-900 dark:text-white font-semibold rounded-lg hover:bg-gray-400 dark:hover:bg-gray-600 transition-colors flex items-center justify-center gap-2">
